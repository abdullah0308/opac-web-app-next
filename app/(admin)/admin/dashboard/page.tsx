    ...
    <div className="p-6 flex flex-col gap-5 stagger">
      ...
      <div className="grid grid-cols-2 gap-3">
        {stats.map(({ label, value, color }) => (
          <div key={label} className="glass-card rounded-[16px] p-4">
            <CountUp value={value} className={`font-mono text-[32px] font-semibold block ${color}`} />
        ...
        <div className="flex flex-col gap-2">
          {[
            ...
            <a key={href} href={href}
              className="group flex items-center justify-between py-2.5 border-b glass-divider last:border-0 transition-colors duration-200 hover:text-opac-green">
              <span className="font-body text-[14px] text-opac-ink">{label}</span>
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                <path d="M6 12L10 8L6 4" stroke="#ADADAA" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="transition-transform duration-200 ease-glide group-hover:translate-x-[3px]"/>
              </svg>
            </a>
          ))}
        </div>
